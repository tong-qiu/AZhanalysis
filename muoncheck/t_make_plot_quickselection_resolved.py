...
if __name__ == '__main__':
    ...
    # Variables to load.
    branches_list_data = [b"EventWeight", b'mVHres', b'nTags', b'ptHcorr']
    # Strings to load.
    ...
    if rescale:
        ...
        for i in range(len(all_sample_after)):
            zpjetlist = [zlib.adler32("Zbb".encode()), zlib.adler32("Zbc".encode()), zlib.adler32("Zcc".encode()), 
                        zlib.adler32("Zcl".encode()), zlib.adler32("Zbl".encode()), zlib.adler32("Zl".encode())]
            mask = np.isin(all_sample_after[i].mata["Sample"], zpjetlist)
            if True in mask:
                all_sample_after[i].rescale(1.301, mask)

            zpjetlist = [zlib.adler32("ttbarB".encode()), zlib.adler32("ttbarC".encode())]
            mask = np.isin(all_sample_after[i].mata["Sample"], zpjetlist)
            if True in mask:
                all_sample_after[i].rescale(1.316, mask)

            zpjetlist = [zlib.adler32("ttbar".encode())]
            mask = np.isin(all_sample_after[i].mata["Sample"], zpjetlist)
            if True in mask:
                all_sample_after[i].rescale(0.904, mask)
